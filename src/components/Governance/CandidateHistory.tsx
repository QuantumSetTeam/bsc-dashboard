...
import { PoolGovAdd, QSDS, QSG } from '../../constants/tokens';
...
async function formatProposals(
    ...
    const currentTotalStake = await getTokenTotalSupply(QSG.addr);
    ...
    const supplyAts = await Promise.all(
        proposals.map(async (p) => {
            ...
            if (epoch > at) {
                return await getTotalBondedAt(PoolGovAdd.addr, at);
            }
            ...
            <DataView
                ...
                renderEntry={(proposal) => [
                    ...
                    <Button
                        ...
                        onClick={() => {
                            history.push(
                                `/governance/candidate/${proposal.candidate}`
                            );
                        }}
                    />,
                ]}
